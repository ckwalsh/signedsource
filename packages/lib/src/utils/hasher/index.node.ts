...
export type { HasherIf } from './common.ts';

export class Hasher implements HasherIf, SyncHasherIf {
  #md5: Hash = createHash('md5');
  #sha256: Hash = createHash('sha256');
  ...
  digest(): Promise<SignedContentHashes> {
    return Promise.resolve(this.syncDigest());
  }
...
const _h: HasherCtor & SyncHasherCtor = Hasher;
